...
### 🔵 新增功能與效能優化 (2026/06)
- **新增 Music SPA 頁面與互動式黑膠播放器**：
  - 在首頁 Header 及手機版選單新增 `MUSIC` 視圖，並嵌入三首 SoundCloud 音軌（*Whisper*, *Electric*, *Generate*）。
  - 設計高質感的**黑膠唱片 (Vinyl Disc) 互動效果**，Hover 時唱片會自封套滑出，點選播放後完全滑出並旋轉。
  - 引入 **SoundCloud Widget API (`api.js`)** 同步唱片旋轉狀態與自訂播放按鈕；實作「單一播放」邏輯（一首播放時暫停其他首）與「導覽跳轉自動暫停」功能，防止背景留聲。
- **2026桃科《自動販賣機》專案套版 (`#project/co3`)**：
  - 更新 Collaboration 3 作品卡片，並新增專屬詳情頁範本 `template-co3`，設計響應式雙欄演職人員名單及 2-3-2 交錯呼吸感劇照網格。
  - 建立圖片 WebP 格式轉換與壓縮腳本 `convert_co3.py`，提升網頁加載速度。
- **細部修正與對外部署優化**：
  - **自訂游標 Iframe 穿透修正**：透過事件委託偵測滑鼠進入 `<iframe>`（YouTube 或 SoundCloud 播放器）內部時，自動隱藏自訂游標並切換為系統預設游標，解決游標卡死在影片外圍邊緣的體驗問題。
  - **Iframe Lazy Loading**：為 Music 視圖下的 iframe 播放器配置 `loading="lazy"` 屬性，減少初始頻寬負載與載入延遲，優化首頁效能。
  - **工作區清理腳本**：建立 `clean_workspace.py`，一鍵自動清除開發測試中產生的多餘臨時程式碼，並在執行後自我刪除以保持專案乾淨。
